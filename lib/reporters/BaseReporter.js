...
BaseReporter.prototype.epilogue = function () {
    ...
    if (stats.failures) {
        format = Colors.wrap(Colors.intenseRed, '  ' + Symbols.error)
            + Colors.wrap(Colors.red, ' %d of %d %s failed')
            + Colors.wrap(Colors.intenseBlack, ':')

        this.writer.write(format, stats.failures, stats.tests, pluralize(stats.tests))
    ...
    this.writer.write(format, stats.tests, pluralize(stats.tests), stats.duration)

    if (stats.skipped) {
        format = Colors.wrap(Colors.cyan, ' ') + Colors.wrap(Colors.cyan, ' %d %s skipped')
        this.writer.write(format, stats.skipped, pluralize(stats.skipped))
    }
