...
BaseReporter.prototype.listFailures = function () {
    ...
    function printError (error) {
        var format = Colors.wrap(Colors.red, '     %s') + Colors.wrap(Colors.intenseBlack, '\n%s\n')
        ...
        if (self.writer.useColors && typeof actual === 'string' && typeof expected === 'string') {
            ...
            format = Colors.wrap(Colors.default, '%s') + Colors.wrap(Colors.intenseBlack, '\n%s\n')
        }

        stack = stack.slice(index ? index + 1 : index).replace(/^/gm, '  ')
        self.writer.write(format, message, stack)
    }
...
BaseReporter.prototype.epilogue = function () {
    ...
    this.writer.write(
        Colors.wrap(Colors.magenta, '  %d max concurrent tests')
