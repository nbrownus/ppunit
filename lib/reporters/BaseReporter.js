...
BaseReporter.prototype.printError = function (error) {
    ...
    if (typeof error.actual === 'string'  && typeof error.expected === 'string') {
        ...
        this.writer.write(this.formatDiff(error, indent))
...
/**
 * Formats an error diff for output
 *
 * @param {Object} error The error that contains the info for the diff
 * @param {String} indent A string to add before each line
 *
 * @returns {string} The formatted diff
 */
BaseReporter.prototype.formatDiff = function (error, indent) {
    var output = []
    ...
    output.push('')
    output.push(indent + Colors.wrap(Colors.bgGreen, '+ expected') + ' ' +  Colors.wrap(Colors.bgRed,   '- actual'))
    output.push('')

    //Get the diff and trim off the header
    var lines = diff.createPatch('string', error.actual, error.expected).split('\n').splice(4)
    output.push(lines.map(cleanUp).filter(notBlank).join('\n'))
    return output.join('\n')
}
