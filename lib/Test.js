...
Test.prototype.resetTimeout = function () {
    ...
    if (this._timeout) {
        this.timer = setTimeout(function () {
            self.complete(new Error('Timeout of ' + ms(self._timeout) + ' exceeded'), Test.RESULT.TIMEOUT)
        }, self._timeout)
    }
}
...
Test.prototype.run = function () {
    ...
    self.dependencies.some(function (test) {
        if (test.result === Test.RESULT.FAILURE || test.result === Test.RESULT.TIMEOUT) {
            ...
            self.complete(new Error(test.fullTitle('/') + ' dependency failed'), Test.RESULT.HOOK_FAILURE)
            return true
        }
    })
...
Test.prototype.complete = function (error, result) {
    ...
    if (self.state === Test.STATE.COMPLETED) {
        ...
        self.result = Test.RESULT.FAILURE
        return
    }
    ...
    //Disposing of the domain on the nextTick seems to fix some issues with nextTick in general
    //TODO: Figure out why this fixes nextTick issues in node 0.8.24 but not node 0.8.2 with domains
//    if (self.domain) {
//        process.nextTick(function () {
//            self.domain.dispose()
//        })
