...
Test.prototype.resetTimeout = function () {
    ...
    if (this._timeout) {
        this.timer = setTimeout(function () {
            self.complete(new Error('Timeout of ' + self._timeout + 'ms exceeded'), Test.RESULT.TIMEOUT)
        }, self._timeout)
    }
}
...
Test.prototype.complete = function (error, result) {
    ...
    //TODO: Figure out why this fixes nextTick issues in node 0.8 with domains
    process.nextTick(function () {
        if (self.domain) {
            self.domain.dispose()
        }
